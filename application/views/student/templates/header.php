...
<head>
	...
	<script>
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			// Saat tombol "Lihat Lokasi" diklik
			document.querySelectorAll('.btn-show-map').forEach(button => {
				button.addEventListener('click', function() {
					const lat = parseFloat(this.getAttribute('data-lat'));
					const lng = parseFloat(this.getAttribute('data-lng'));

					// Cek apakah lat dan lng valid
					if (!isNaN(lat) && !isNaN(lng)) {
						const locationModal = new bootstrap.Modal(document.getElementById('locationModal'));
						locationModal.show(); // Memastikan modal ditampilkan

						// Tunggu sampai modal benar-benar muncul, lalu inisialisasi peta
			...
			modalCloseBtn.forEach(btn => {
				btn.addEventListener('click', function() {
					modal.hide(); // Menutup modal secara manual
				});
			});
		});
	</script>
